...
namespace ProseTutorial
{
    ...
    {
        private const string GrammarPath = @"../../../../ProseTutorial/synthesis/grammar/concat.grammar";

        [TestMethod]
        public void TestLearnConcatString()
        {
            //parse grammar file 
            ...
            //create the example
            State input = State.CreateForExecution(grammar.Value.InputSymbol, "(Feb 1960)");
            var examples = new Dictionary<State, object> { { input, "1960 Feb" } };
            //State secondInput = State.CreateForExecution(grammar.Value.InputSymbol, "(Jan 2012)");
            //State thirdInput = State.CreateForExecution(grammar.Value.InputSymbol, "(Mar 2020)");
            //var examples = new Dictionary<State, object> { { input, "1960 Feb" }, { secondInput, "2012 Jan" }, { thirdInput, "2020 Mar" } };
            ...
            Console.Out.WriteLine(programs.First().PrintAST(ASTSerializationFormat.HumanReadable));
            //foreach (ProgramNode program in programs)
            //{
            ...
            Assert.AreEqual("1960 Feb", output);

            //State differentInput = State.CreateForExecution(grammar.Value.InputSymbol, "(Jan 2000)");
            //output = programs.First().Invoke(differentInput) as string;
            //Assert.AreEqual("2000 Jan", output);
        }
        ...
        //    var examples = new Dictionary<State, object> { { input, "19-Feb-1960" } };
        //    var spec = new ExampleSpec(examples);
